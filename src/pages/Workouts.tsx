...
const Workouts = () => {
  ...
  const addExercise = () => {
    setWorkout(old => ({
      ...
      exercises: [
        ...old.exercises,
        { id: crypto.randomUUID(), name: '', sets: '', reps: '', rest: '0s' }
      ]
    }))
  }
            ...
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                <Field>
                  ...
                  <Input
                    value={workout.name}
                    onChange={e => setWorkout(old => ({ ...old, name: e.target.value }))}
                  ...
                  <>
                    <div className="flex flex-col gap-2 max-h-[295px] overflow-y-auto scrollbar-hide pb-1">
                      {workout.exercises.map((ex, idx) => (
                        <div key={ex.id} className="flex items-center gap-3 p-3 bg-muted/20 border rounded-xl shrink-0">
                          ...
                          <Input
                            ...
                            className="border-none bg-transparent shadow-none focus-visible:ring-0 p-0 font-medium"
                          />
                          <div className="flex items-center gap-2">
                            <Input value={ex.sets} onChange={e => updateExercise(ex.id, 'sets', e.target.value)} className="w-12 h-8 text-center bg-muted" />
                            <span className="text-muted-foreground/40 text-xs">×</span>
                            <Input value={ex.reps} onChange={e => updateExercise(ex.id, 'reps', e.target.value)} className="w-12 h-8 text-center bg-muted" />
                            <Input value={ex.rest} onChange={e => updateExercise(ex.id, 'rest', e.target.value)} className="w-16 h-8 text-center bg-card border rounded-md text-[10px] font-bold text-muted-foreground" placeholder="90s" />
            ...
            <Tabs defaultValue="all">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                <TabsList>
                  ...
                  <TabsTrigger value="upper-body">Upper</TabsTrigger>
                  <TabsTrigger value="lower-body">Lower</TabsTrigger>
                  <TabsTrigger value="push">Push</TabsTrigger>
                  <TabsTrigger value="pull">Pull</TabsTrigger>
                  <TabsTrigger value="full-body">Full</TabsTrigger>
                </TabsList>
              ...
              <TabsContent value="all">{renderList(filteredAll)}</TabsContent>
              <TabsContent value="upper-body">{renderList(getByCategory('upper-body'))}</TabsContent>
              <TabsContent value="push">{renderList(getByCategory('push'))}</TabsContent>
              <TabsContent value="pull">{renderList(getByCategory('pull'))}</TabsContent>
              <TabsContent value="lower-body">{renderList(getByCategory('lower-body'))}</TabsContent>
              <TabsContent value="full-body">{renderList(getByCategory('full-body'))}</TabsContent>
            </Tabs>
          )}
        </div>
      </div>
