...
export function CardWorkout({
  ...
  return (
    <div className="flex flex-col p-5 border rounded-2xl bg-card hover:shadow-sm transition-all text-left" onClick={onClick}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="icon" className="h-9 w-9 rounded-xl border-muted-foreground/20" onClick={e => e.stopPropagation()}>
            <CalendarDays size={16} className="text-muted-foreground" />
          </Button>
          <Button size="sm" className="h-9 rounded-full px-5 gap-1.5 bg-foreground text-background hover:bg-foreground/90 font-semibold" onClick={e => e.stopPropagation()}>
